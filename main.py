...
def calculate_mood_deltas(agent_name: str, match_data: dict) -> dict:
    """
    Calculate how mood scores should shift after this match.
    Based on whether the agent's team won, lost, drew, or wasn't playing.
    """
    country_map = {
        "cadu": "Brazil",
        "rodrigo": "Argentina",
...
def run_session():
    ...
    This is what runs autonomously from June 11 onwards.
    """
    ...
    if not match_data:
        print("⚠️  No FIFA 2026 match available. Nothing to run.")
    ...
    if already_ran(match_id):
        ...
        print("The bar already discussed this one. Come back after the next match.")
        sys.exit(0)
    ...
    print(f"📊 Total sessions this tournament: {get_session_count()}")

    if bartender_mode == "prediction":
        print("\n🍺 THE BARTENDER HAS SPOKEN. Prediction saved.")
...
def run_test_session():
    ...
    match_data = {
        ...
        "away_score": 1,
        "goals": [
            {"minute": 34, "team": "Brazil", "scorer": "Vinicius Jr"},
            {"minute": 61, "team": "France", "scorer": "Mbappé"},
            {"minute": 88, "team": "Brazil", "scorer": "Rodrygo"}
        ],
        "red_cards": [],
        "stage": "Group Stage",
...
Venue: MetLife Stadium, New Jersey
Date: 2026-06-07
Scorers: Vinicius Jr 34' (Brazil) | Mbappé 61' (France) | Rodrygo 88' (Brazil)
Red Cards: None
"""
    }
    ...
    if already_ran(match_id):
        print("⚠️  Test session already ran. Check memory/cadu.json to confirm diary updated.")
        return
    ...
    task_cadu = Task(
        description=f"The match just finished: {match_context}\nYou are in The Neutral Zone. React. Be Cadu. 2-4 sentences.",
        expected_output="Cadu's reaction",
        agent=cadu
    )
    task_rodrigo = Task(
        description=f"The match just finished: {match_context}\nCadu just spoke. React. Be Rodrigo. 2-4 sentences.",
        expected_output="Rodrigo's reaction",
        agent=rodrigo,
        context=[task_cadu]
    )
    task_gary = Task(
        description=f"The match just finished: {match_context}\nCadu and Rodrigo spoke. React. Be Gary. 2-4 sentences.",
        expected_output="Gary's reaction",
        agent=gary,
        context=[task_cadu, task_rodrigo]
    )
    task_klaus = Task(
        description=f"The match just finished: {match_context}\nEveryone spoke. Analyse. Be Klaus. 2-4 sentences.",
        expected_output="Klaus's reaction",
        agent=klaus,
        context=[task_cadu, task_rodrigo, task_gary]
    )
    task_antoine = Task(
        description=f"The match just finished: {match_context}\nEveryone spoke. React philosophically. Be Antoine. 2-4 sentences.",
        expected_output="Antoine's reaction",
        agent=antoine,
        context=[task_cadu, task_rodrigo, task_gary, task_klaus]
    )
    task_bartender = Task(
        description="Watch the session. End with one quiet observation. One or two sentences.",
        expected_output="Bartender's closing line",
        agent=bartender,
        context=[task_cadu, task_rodrigo, task_gary, task_klaus, task_antoine]
    )
    ...
    crew = Crew(
        agents=[cadu, rodrigo, gary, klaus, antoine, bartender],
        tasks=[
            task_cadu, task_rodrigo, task_gary,
            task_klaus, task_antoine, task_bartender
        ],
        process=Process.sequential,
        verbose=False
    )
    ...
    print("✅ Test session complete. Memories updated.")
